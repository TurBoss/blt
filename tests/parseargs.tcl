...
test parseargs.259 {myParser add test} {
...
test parseargs.291 {myParser parse "--debug 1000"} {
    list [catch { myParser parse "--debug 1000" } msg] $msg
} {0 {}}

test parseargs.292 {myParser parse "--debug -1000"} {
    list [catch { myParser parse "--debug -1000" } msg] $msg
} {0 {}}

test parseargs.293 {myParser parse "--debug -next"} {
    list [catch { myParser parse "--debug -next" } msg] $msg
} {1 {unknown option "-next"}}

test parseargs.294 {myParser parse "-d -1,0,1,2"} {
    list [catch { myParser parse "-d -1,0,1,2" } msg] $msg
} {1 {expected boolean value but got "-1,0,1,2": bad value for "--debug"}}

test parseargs.295 {myParser argument configure "verbose" -min 100 -max 10} {
    list [catch { 
	myParser argument configure "verbose" -min 100 -max 10 
    } msg] $msg
} {0 {}}

test parseargs.296 {myParser argument configure "verbose" -min 1 -max 10} {
    list [catch { 
	myParser argument configure "verbose" -min 1 -max 10 
    } msg] $msg
} {0 {}}

test parseargs.297 {myParser argument configure "verbose" -min abc -max def} {
    list [catch { 
...
test parseargs.299 {myParser configure -error ""} {
...
test parseargs.318 {myParser parse "-type 1 -type 5 -type 10"} {
    list [catch { myParser parse "-type 1 -type 5 -type 10" } msg] $msg
} {0 {}}

test parseargs.319 {myParser get "type"} {
    list [catch { myParser get "type" } msg] $msg
} {0 10}


test parseargs.320 {myParser argument configure "type" -action append} {
    list [catch { myParser argument configure "type" -action append } msg] $msg
} {0 {}}

test parseargs.321 {myParser get "type"} {
    list [catch { myParser get "type" } msg] $msg
} {0 10}

test parseargs.322 {myParser parse "-type 1 -type 5 -type 10"} {
    list [catch { myParser parse "-type 1 -type 5 -type 10" } msg] $msg
} {0 {}}
...
test parseargs.324 {myParser argument configure "type" -action store} {
    list [catch { myParser argument configure "type" -action store } msg] $msg
} {0 {}}

test parseargs.325 {myParser parse "-type 1e1"} {
    list [catch { myParser parse "-type 1e1" } msg] $msg
} {0 {}}
...
test parseargs.327 {myParser argument configure "type" -action store_true} {
    list [catch {
	myParser argument configure "type" -action store_true
    } msg] $msg
} {0 {}}

test parseargs.328 {myParser parse "-type 5"} {
    list [catch { myParser parse "-type 5" } msg] $msg
} {0 {}}
...
test parseargs.330 {myParser argument configure "type" -action store_false} {
    list [catch {
	myParser argument configure "type" -action store_false
    } msg] $msg
} {0 {}}

test parseargs.331 {myParser parse "-type 5"} {
    list [catch { myParser parse "-type 5" } msg] $msg
} {0 {}}
...
test parseargs.335 {myParser argument configure "type" -variable badVar} {
    list [catch {
	myParser argument configure "type" -variable ::arrayvar
    } msg] $msg
} {0 {}}

test parseargs.336 {myParser parse "-type 5"} {
    list [catch { myParser parse "-type 5" } msg] $msg
} {1 {can't set "::arrayvar": variable is array}}

test parseargs.337 {myParser argument configure "type" -variable ""} {
    list [catch {
...
test parseargs.338 {myParser add date} {
    list [catch { 
...
test parseargs.339 {myParser parse "-date 1/1/1970"} {
    list [catch { myParser parse "-date 1/1/1970" } msg] $msg
} {0 {}}


test parseargs.340 {myParser get "date"} {
...
test parseargs.342 {myParser reset} {
    list [catch { myParser reset } msg] $msg
} {0 {}}

test parseargs.343 {myParser parse "-date 1/1/1970"} {
    list [catch { myParser parse "-date 1/1/1970" } msg] $msg
} {1 {missing required argument "-type"}}

test parseargs.344 {myParser get "date"} {
    list [catch { myParser get "date" } msg] $msg
} {0 0}

test parseargs.345 {myParser exists "date"} {
    list [catch { myParser exists "date" } msg] $msg
} {0 1}

test parseargs.346 {myParser exists "type"} {
    list [catch { myParser exists "type" } msg] $msg
} {0 1}

test parseargs.347 {myParser delete (no arg)} {
    list [catch { myParser delete } msg] $msg
} {0 {}}

test parseargs.348 {myParser delete badArg} {
    list [catch { myParser delete badArg } msg] $msg
} {1 {can't find argument "badArg" in parser "::myParser"}}

test parseargs.349 {myParser delete "type" "verbose" } {
    list [catch { myParser delete "type" "verbose" } msg] $msg
} {0 {}}

test parseargs.350 {myParser exists "type"} {
    list [catch { myParser exists "type" } msg] $msg
} {0 0}

test parseargs.351 {myParser get} { 
    list [catch { myParser get } msg] $msg 
} {0 {debug 0 date 0}}

test parseargs.352 {myParser parse "-date 1/2/1970 e f g"} {
    list [catch { 
	myParser parse "-date 1/2/1970 e f g" 
    } msg] $msg
} {0 {e f g}}

test parseargs.353 {myParser parse "a b c -date 1/2/1970 e f g"} {
    list [catch { 
	myParser parse "a b c -date 1/2/1970 e f g" 
    } msg] $msg
} {0 {a b c e f g}}

test parseargs.354 {myParser parse "--debug a b c -date 1/2/1970 e f g"} {
    list [catch { 
	myParser parse "a b --debug 0 c d -date 1/2/1970 e f g" 
    } msg] $msg
} {0 {a b c d e f g}}

test parseargs.355 {myParser add files} {
    list [catch { 
	myParser add files -short -f -long -files -nargs * -metavar file
    } msg] $msg
} {0 files}

test parseargs.356 {myParser parse "-f"} {
    list [catch { myParser parse "-f" } msg] $msg
} {0 {}}

test parseargs.357 {myParser get files} { 
    list [catch { myParser get files } msg] $msg 
} {0 {}}

test parseargs.358 {myParser parse "-f a b c"} {
    list [catch { myParser parse "-f a b c" } msg] $msg
} {0 {}}

test parseargs.359 {myParser get files} { 
    list [catch { myParser get files } msg] $msg 
} {0 {a b c}}

test parseargs.360 {myParser parse "--debug -f a b c"} {
    list [catch { myParser parse "--debug -f a b c" } msg] $msg
} {0 {}}

test parseargs.361 {myParser parse "-f a b c -date 1/2/1970 e f g"} {
    list [catch { 
	myParser reset
...
test parseargs.362 {myParser parse "0 1 2 3 --debug -f a b c -date 1/1/1970 e f g"} {
    list [catch { 
	myParser reset
...
test parseargs.363 {myParser parse "0 1 2 3 --debug -f -date 1/1/1970 e f g"} {
    list [catch { 
	myParser reset
...
test parseargs.364 {myParser parse "0 1 2 3 --debug -f c -date 1/1/1970 e f g"} {
    list [catch { 
	myParser reset
...
test parseargs.365 {myParser parse "0 1 2 3 --debug -f c d -date 1/1/1970 e f g"} {
    list [catch { 
	myParser reset
...
test parseargs.366 {myParser get} {
    list [catch { myParser get } msg] $msg
} {0 {debug 0 date 0 files {c d}}}

test parseargs.367 {myParser argument configure files -nargs 3} {
    list [catch { myParser argument configure files -nargs 3 } msg] $msg
} {0 {}}

test parseargs.368 {myParser parse "-f a b c"} {
    list [catch { 
	myParser reset
...
test parseargs.369 {myParser parse "-f a b c -d"} {
    list [catch { 
	myParser reset
...
test parseargs.370 {myParser parse "-f a b c d"} {
    list [catch { 
	myParser reset
	myParser parse "-f a b c d" 
    } msg] $msg
...
test parseargs.371 {myParser parse "-f"} {
    list [catch { 
	myParser reset
	myParser parse "-f" 
    } msg] $msg
...
test parseargs.372 {myParser parse "-f a"} {
    list [catch { 
	myParser reset
	myParser parse "-f a" 
    } msg] $msg
...
test parseargs.373 {myParser parse "-f a b"} {
    list [catch { 
	myParser reset
...
test parseargs.375 {myParser parse "-d 0"} {
    list [catch { 
	myParser reset
	myParser parse "-d 0" 
    } msg] $msg
...
test parseargs.376 {myParser parse "0 1 2 3 --debug -f c d -date 1/1/1970 e f g"} {
    list [catch { 
	myParser reset
	myParser parse "0 1 2 3 --debug -f c d -date 1/1/1970 e f g" 
    } msg] $msg
...
test parseargs.377 {myParser argument configure files -nargs +} {
    list [catch { myParser argument configure files -nargs + } msg] $msg
} {0 {}}

test parseargs.378 {myParser parse "-f a b c d"} {
    list [catch { 
	myParser reset
	myParser parse "-f a b c d" 
    } msg] $msg
...
test parseargs.379 {myParser parse "-f"} {
    list [catch { 
	myParser reset
	myParser parse "-f" 
    } msg] $msg
...
test parseargs.380 {myParser parse "-f a"} {
    list [catch { 
	myParser reset
	myParser parse "-f a" 
    } msg] $msg
...
test parseargs.381 {myParser parse "-f a b"} {
    list [catch { 
	myParser reset
	myParser parse "-f a b" 
    } msg] $msg
...
test parseargs.382 {myParser parse "-f a b c"} {
    list [catch { 
	myParser reset
	myParser parse "-f a b c" 
    } msg] $msg
...
test parseargs.383 {myParser parse "-f a b c -d"} {
    list [catch { 
	myParser reset
	myParser parse "-f a b c -d" 
    } msg] $msg
...
test parseargs.384 {myParser get files} { 
    list [catch { myParser get files } msg] $msg 
} {0 {a b c}}

test parseargs.385 {myParser argument configure files -nargs 0} {
    list [catch { myParser argument configure files -nargs 0 } msg] $msg
} {0 {}}

test parseargs.386 {myParser parse "-f a b c d"} {
    list [catch { 
	myParser reset
	myParser parse "-f a b c d" 
    } msg] $msg
...
test parseargs.387 {myParser parse "-f"} {
    list [catch { 
	myParser reset
	myParser parse "-f" 
    } msg] $msg
...
test parseargs.388 {myParser parse "-f a"} {
    list [catch { 
	myParser reset
	myParser parse "-f a" 
    } msg] $msg
...
test parseargs.389 {myParser parse "-f a b"} {
    list [catch { 
	myParser reset
	myParser parse "-f a b" 
    } msg] $msg
...
test parseargs.390 {myParser parse "-f a b c"} {
    list [catch { 
	myParser reset
	myParser parse "-f a b c" 
    } msg] $msg
...
test parseargs.391 {myParser parse "-f a b c -d"} {
    list [catch { 
	myParser reset
	myParser parse "-f a b c -d" 
    } msg] $msg
...
test parseargs.392 {myParser get files} { 
    list [catch { myParser get files } msg] $msg 
} {0 {}}

test parseargs.393 {myParser add things} {
    list [catch { 
	myParser add things -nargs *
    } msg] $msg
} {0 things}

test parseargs.394 {myParser parse "a b c -d"} {
    list [catch { 
	myParser reset
...
test parseargs.395 {myParser get } { 
    list [catch { myParser get } msg] $msg 
} {0 {debug 0 date {} files {} things {a b c}}}

test parseargs.396 {myParser parse "-f a b c -d"} {
    list [catch { 
	myParser reset
	myParser parse "-f a b c -d" 
    } msg] $msg
...
test parseargs.397 {myParser get } { 
    list [catch { myParser get } msg] $msg 
} {0 {debug 0 date {} files {} things {a b c}}}

test parseargs.398 {myParser parse ""} {
    list [catch { 
	myParser reset
	myParser parse "" 
    } msg] $msg
...
test parseargs.399 {myParser get } { 
    list [catch { myParser get } msg] $msg 
} {0 {debug 0 date {} files {} things {}}}

test parseargs.400 {myParser parse "a"} {
    list [catch { 
	myParser reset
	myParser parse "a" 
    } msg] $msg
...
test parseargs.402 {myParser parse "a b -d"} {
    list [catch { 
	myParser reset
...
test parseargs.404 {myParser delete things } {
    list [catch { myParser delete things } msg] $msg
} {0 {}}

test parseargs.405 {myParser add first } {
    list [catch { myParser add first } msg] $msg
} {0 first}

test parseargs.406 {myParser add second } {
    list [catch { myParser add second } msg] $msg
} {0 second}

test parseargs.407 {myParser add third } {
    list [catch { myParser add third } msg] $msg
} {0 third}

test parseargs.408 {myParser parse ""} {
    list [catch { 
	myParser reset
	myParser parse "" 
    } msg] $msg
...
test parseargs.410 {myParser parse "a"} {
    list [catch { 
	myParser reset
	myParser parse "a" 
    } msg] $msg
...
test parseargs.412 {myParser parse "a b"} {
    list [catch { 
	myParser reset
	myParser parse "a b" 
    } msg] $msg
...
test parseargs.414 {myParser parse "a b c"} {
    list [catch { 
	myParser reset
...
test parseargs.416 {myParser parse "a b c d"} {
    list [catch { 
	myParser reset
	myParser parse "a b c d" 
    } msg] $msg
...
test parseargs.418 {myParser argument configure files -nargs 0} {
    list [catch { myParser argument configure files -nargs + } msg] $msg
} {0 {}}

test parseargs.419 {myParser add fahrenheit} {
    list [catch {
...
test parseargs.420 {myParser add centigrade} {
    list [catch {
...
test parseargs.421 {myParser parse "--fahrenheit"} {
    list [catch { 
	myParser reset
...
test parseargs.422 {myParser parse "--centigrade"} {
    list [catch { 
	myParser reset
...
test parseargs.423 {myParser parse "--fahrenheit --centigrade"} {
    list [catch { 
	myParser reset
...
test parseargs.424 {myParser parse "--centigrade --fahrenheit"} {
    list [catch { 
	myParser reset
...
test parseargs.425 {myParser delete "fahrenheit" "centigrade" } {
    list [catch { myParser delete "fahrenheit" "centigrade" } msg] $msg
} {0 {}}

test parseargs.426 {myParser add fahrenheit} {
    list [catch {
	myParser add fahrenheit -long --fahrenheit -type boolean -nargs 0 \
...
test parseargs.427 {myParser add centigrade} {
    list [catch {
	myParser add centigrade -long --centigrade -type boolean -nargs 0 \
	    -action store_true -default 0 -exclude badArg
    } msg] $msg
...
test parseargs.428 {myParser parse "--fahrenheit"} {
    list [catch { 
	myParser reset
	myParser parse "--fahrenheit" 
    } msg] $msg
...
test parseargs.429 {myParser parse "--centigrade"} {
    list [catch { 
	myParser reset
	myParser parse "--centigrade" 
    } msg] $msg
...
test parseargs.430 {myParser parse "--fahrenheit --centigrade"} {
    list [catch { 
	myParser reset
	myParser parse "--fahrenheit --centigrade" 
    } msg] $msg
...
test parseargs.431 {myParser parse "--centigrade --fahrenheit"} {
    list [catch { 
	myParser reset
	myParser parse "--centigrade --fahrenheit" 
    } msg] $msg
...
test parseargs.432 {myParser delete "fahrenheit" "centigrade" } {
    list [catch { myParser delete "fahrenheit" "centigrade" } msg] $msg
} {0 {}}

test parseargs.433 {myParser add test} {
    list [catch {
...
test parseargs.434 {myParser parse ""} {
    list [catch { 
	myParser reset
	myParser parse "" 
    } msg] $msg
...
test parseargs.435 {myParser get test} {
    list [catch { myParser get test } msg] $msg
} {0 NA}

test parseargs.436 {myParser parse "-t"} {
    list [catch { 
	myParser reset
	myParser parse "-t" 
    } msg] $msg
...
test parseargs.437 {myParser get test} {
    list [catch { myParser get test } msg] $msg
} {0 10}

test parseargs.438 {myParser configure test -value NA} {
    list [catch {
	myParser argument configure test -value NA 
    } msg] $msg
} {0 {}}

test parseargs.439 {myParser parse "-t"} {
    list [catch { 
	myParser reset
	myParser parse "-t" 
    } msg] $msg
...
test parseargs.441 {myParser configure test -value 0} {
    list [catch {
	myParser argument configure test -value 0
    } msg] $msg
} {0 {}}

test parseargs.442 {myParser delete first second third} {
    list [catch { myParser delete first second third } msg] $msg
} {0 {}}

test parseargs.443 {myParser add --} {
    list [catch { myParser add -- -short -- -nargs last } msg] $msg
} {0 --}

test parseargs.444 {myParser parse "-t -- -t --debug --files" } {
    list [catch { 
	myParser reset
...
test parseargs.445 {myParser parse "rm -- -i -r" } {
    list [catch { 
	myParser reset
	myParser parse "-- -i -r" 
    } msg] $msg
...
test parseargs.446 {myParser get} {
    list [catch { myParser get } msg] $msg
} {0 {debug 0 date {} files {} test NA -- {}}}

test parseargs.447 {myParser parse "rm -- -i -r" } {
    list [catch { 
	myParser reset
	myParser parse "-- -i -r" 
    } msg] $msg
...
test parseargs.448 {myParser argument configure test} {
    list [catch { 
...
test parseargs.449 {myParser parse "-t -d --debug -f -files" } {
    list [catch { 
	myParser reset
...
test parseargs.505 {myParser parse "-t no"} {
    list [catch { myParser parse "-t no" } msg] $msg
} {0 {}}

test parseargs.506 {myParser parse "-t 0"} {
    list [catch { myParser parse "-t 0" } msg] $msg
} {0 {}}

test parseargs.507 {myParser argument configure temp} {
    list [catch { 
	myParser argument configure temp -type double -action append 
    } msg] $msg
} {0 {}}

test parseargs.508 {myParser parse "-t 10 -t 20 -t 30"} {
    list [catch { myParser parse "-t 10 -t 20 -t 30" } msg] $msg
} {0 {}}

test parseargs.509 {myParser get temp} {
    list [catch { myParser get temp} msg] $msg
} {0 {0 10 20 30}}


test parseargs.510 {myParser reset } {
    list [catch { myParser reset } msg] $msg
} {0 {}}

test parseargs.511 {myParser get temp} {
    list [catch { myParser get temp } msg] $msg
} {0 120}
...
test parseargs.513 {myParser parse "-t 1.0 -t 2.0 -t 3.0"} {
...
test parseargs.515 {myParser delete temp} {
    list [catch { myParser delete temp } msg] $msg
} {0 {}}

test parseargs.516 {myParser add temp} {
    list [catch { 
...
test parseargs.527 {myParser parse "-s abc -i 10 -f 3.1415 -b yes"} {
    list [catch { myParser parse "-s abc -i 10 -f 3.1415 -b yes"} msg] $msg
} {0 {}}

test parseargs.528 {myParser get string} {
    list [catch { myParser get string } msg] $msg
} {0 {abc 10 3.1415 yes}}

test parseargs.529 {myParser get boolean} {
    list [catch { myParser get boolean } msg] $msg
} {0 {abc 10 3.1415 yes}}

test parseargs.530 {myParser get int} {
    list [catch { myParser get int } msg] $msg
} {0 {abc 10 3.1415 yes}}

test parseargs.531 {myParser get float} {
    list [catch { myParser get float } msg] $msg
} {0 {abc 10 3.1415 yes}}

test parseargs.532 {myParser delete string} {
    list [catch { myParser delete string } msg] $msg
} {0 {}}

test parseargs.533 {myParser parse "-i 10000"} {
    list [catch { myParser parse "-i 10000"} msg] $msg
} {0 {}}

test parseargs.534 {myParser get int} {
    list [catch { myParser get int } msg] $msg
} {0 10000}

test parseargs.535 {myParser get float} {
    list [catch { myParser get float } msg] $msg
} {0 {}}

test parseargs.536 {myParser parse "-f 0.01"} {
    list [catch { myParser parse "-f 0.01"} msg] $msg
} {0 {}}

test parseargs.537 {myParser get float} {
    list [catch { myParser get float } msg] $msg
} {0 0.01}

test parseargs.538 {myParser configure -prefixchar /} {
...
test parseargs.540 {myParser configure -error "badoption extraargs" } {
    list [catch { myParser configure -error "badoption extraargs" } msg] $msg
} {0 {}}

test parseargs.541 {myParser parse "-s abc -i 10 -f 3.1415 -b yes"} {
    list [catch { myParser parse "-s abc -i 10 -f 3.1415 -b yes"} msg] $msg
} {1 {unknown arguments found}}

test parseargs.542 {myParser parse "/i 10 /f 3.1415 /b yes"} {
    list [catch { myParser parse "/i 10 /f 3.1415 /b yes"} msg] $msg
} {1 {unknown option "/i"}}

test parseargs.543 {myParser configure int -short /i -long /int} {
    list [catch { myParser argument configure int -short /i -long /int } msg] $msg
} {0 {}}

test parseargs.544 {myParser parse "/i 10"} {
    list [catch { myParser parse "/i 10"} msg] $msg
} {0 {}}

test parseargs.545 {myParser get int} {
    list [catch { myParser get int } msg] $msg
} {0 {10000 10}}

test parseargs.546 {myParser get string defValue} {
